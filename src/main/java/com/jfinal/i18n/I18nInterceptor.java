...
public class I18nInterceptor implements Interceptor {
	...
	private boolean isSwitchView = true;		//liupeng update:set default value to true
	...
	public void intercept(Invocation inv) {
		...
		if (StrKit.notBlank(locale)) {	// change locale, write cookie
			...
			// liupeng update:set locale by request's locale
			if (StrKit.isBlank(locale)) {
				locale = c.getRequest().getLocale().getLanguage();
				//非中文语言都默认为英文
				if (!locale.equalsIgnoreCase(Locale.CHINESE.getLanguage())) {
					locale = "en";
				}
				c.setCookie(localePara, locale, Const.DEFAULT_I18N_MAX_AGE_OF_COOKIE);
			}
	 ...
	 */
	public void switchView(String locale, Invocation invcation) {
		Render render = invcation.getController().getRender();
		if (render != null) {
			String view = render.getView();
			if (view != null) {
				// liupeng update:set view path to /locale/controllerkey/view
				if (!locale.equalsIgnoreCase(Locale.CHINESE.getLanguage())) {
					if (view.startsWith("/"))
						view = "/" + locale + view;
					else
						view = "/" + locale + invcation.getControllerKey() + "/" + view;
				}
