...
public class PoiImporter {
    ...
    public static List<List<List<String>>> readExcel(File file, com.jfinal.ext.kit.excel.Rule rule) {
...
//                            format.setGroupingUsed(false);
    ...
    public static Model<?> fillModel(Class<?> clazz, List<String> list, Rule rule) {
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            com.jfinal.ext.kit.excel.Rule.Cell cell = matchCell(rule, i);
            ...
            if (valid) {
                ...
                if (StrKit.notBlank(convertClassName)) {
                    CellConvert<Object> cellConvert = Reflect.on(convertClassName).get();
    ...
    public static com.jfinal.ext.kit.excel.Rule.Cell matchCell(Rule rule, int index) {
        List<com.jfinal.ext.kit.excel.Rule.Cell> cells = rule.getCells();
        for (int i = 0; i < cells.size(); i++) {
            com.jfinal.ext.kit.excel.Rule.Cell cell = cells.get(i);
